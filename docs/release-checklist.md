...
Use this checklist from the exact commit proposed for release. The package remains private to npm; version `0.2.0` is distributed only through GitHub Releases.

## Release candidate

- [ ] Confirm `package.json` version `0.2.0` maps exactly to tag `v0.2.0`.
...
- [ ] Create and push only the annotated `v0.2.0` tag at the frozen release commit.
